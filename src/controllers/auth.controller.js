...
const mapDashboardUser = (user) => ({
	uslogin: user.uslogin ?? user.id ?? null,
	ustipo: user.ustipo ?? user.rol ?? null,
	nombre: user.nombre ?? null,
	appaterno: user.appaterno ?? null,
	apmaterno: user.apmaterno ?? null,
	email: user.email ?? null,
	usultimoacceso: user.usultimoacceso ?? null,
	dias: user.dias ?? null,
});
...
export const login = async (req, res) => {
	try {
		...
		createSession({
			res,
			payload: mapDashboardUser(user),
		});

		return res.json({
			success: true,
			user: mapDashboardUser(user),
		});
	} catch (error) {
...
export const getMe = async (req, res) => {
	try {
		...
		return res.json({
			success: true,
			user: mapDashboardUser(req.user),
		});
	} catch (error) {
